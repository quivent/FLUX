...
def load_plan(path):
    with open(path, "r", encoding="utf-8") as f:
        plan = json.load(f)
    if not plan.get("source_image"):
        raise ValueError("plan needs source_image")
...
def expand_jobs(plan):
    ...
    base = {
        "backend": plan.get("backend") or "auto",
        "width": int(plan.get("width") or 0),
        "height": int(plan.get("height") or 0),
        "steps": int(plan.get("steps") or 18),
        "guidance": float(plan.get("guidance") or 5.5),
        "strength": float(plan.get("strength") or 0.55),
        "image": str(plan["source_image"]),
    }
    jobs = []
    ...
    while True:
        ...
        if job:
            status_path.write_text(
                json.dumps({"batch": batch_row, "worker_job": job, "updated": time.time()}, indent=2, sort_keys=True) + "\n",
                encoding="utf-8",
            )
        ...
        with completed_path.open("a", encoding="utf-8") as completed:
            for index, job in enumerate(jobs, start=1):
                ...
                payload = {
                    ...
                    "conditioning": "image-latent anchored motion source",
                }
...
def main():
    parser = argparse.ArgumentParser(description="Submit an image-conditioned FLUX batch through the img2img socket.")
    parser.add_argument("plan")
    parser.add_argument("--socket", default=".fluxd/img2img.sock")
