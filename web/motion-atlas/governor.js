...
var DEFAULT_ENDPOINT = window.location.origin;
    ...
    <div class="shell">
      <aside class="rail">
        <div class="mark">
          ...
          <div class="mark-sub">v${"motion-atlas"}</div>
        </div>
...
var stored = localStorage.getItem("motion-atlas:governor-messages");
var messages2 = stored ? JSON.parse(stored) : [];
var promptText2 = "";
var busy2 = false;
var statusText2 = "Ready";
var lastError2 = "-";
...
function persist() {
  localStorage.setItem("motion-atlas:governor-messages", JSON.stringify(messages2));
}
function render() {
...
async function send(rawPrompt) {
  const content = rawPrompt.trim();
  if (!content || busy2) return;
  const started = performance.now();
  messages2 = [...messages2, { role: "user", content, at: timeNow() }, { role: "assistant", content: "", at: timeNow() }];
  promptText2 = "";
  busy2 = true;
  statusText2 = "Waiting for Governor";
  lastError2 = "-";
  persist();
  render();
  abortController = new AbortController();
  try {
    const response = await fetch("/api/governor/chat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        model: "governor",
        messages: messages2.filter((message) => message.content).map(({ role, content: content2 }) => ({ role, content: content2 })),
        temperature: 0.2,
        max_tokens: 4096,
        stream: false
      }),
    ...
    lastError2 = message;
    statusText2 = stopped ? "Request stopped" : "Governor unavailable";
  } finally {
    busy2 = false;
