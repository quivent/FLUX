...
function connectSocket(path,onData){const url=(location.protocol==="https:"?"wss://":"ws://")+location.host+path;let ws,retries=0;const connect=()=>{ws=new WebSocket(url);ws.onopen=()=>{retries=0};ws.onmessage=e=>{let data=null;try{data=JSON.parse(e.data)}catch{}if(data)onData(data)};ws.onerror=()=>{ws.close()};ws.onclose=()=>{const delay=Math.min(1000*2**retries,10000)+Math.random()*300;retries++;setTimeout(connect,delay)}};connect()}
connectSocket("/api/assets/ws",event=>{const a=event.asset||{};if(!String(a.name||"").startsWith("optical-proof-"))return;const img=document.createElement("img");img.src=a.access_url;img.title=`Seed ${a.seed||"—"}`;$("opticAssets").prepend(img);$("opticState").textContent="PIPER · OPTICAL PROOF RECEIVED"});
connectSocket("/api/model/ws",m=>{const top=$("modelTopState");top.className="modelTopState "+(m.loaded?"loaded":m.downloaded?"downloaded":"");top.lastChild.textContent=m.loaded?" MODEL LOADED":m.downloaded?" MODEL READY":" MODEL MISSING"});
