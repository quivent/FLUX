...
function sphereOrder(job){
  ...
  const loop=["elliptic","sway","oscillatory","screw"].includes(String(job.mode||"").toLowerCase()),scale=Number(job.shell_scale)||1,lock=Number(job.seed_lock)||0;
...
function drawSphere(){
  ...
  const done=Number(sphere.job?.step||sphere.job?.atlas_done||0),sites=(Number(sphere.job?.n_rows)||1024)*(Number(sphere.job?.n_cols)||64),loop=!!sphere.points[0]?.loop,rho=clamp(done/sites,0,1),fillR=r*rho;if(!loop&&fillR>0){c.beginPath();c.arc(cx,cy,fillR,0,Math.PI*2);c.fillStyle="rgba(109,241,255,.42)";c.fill();if(fillR>r*.02){c.beginPath();c.arc(cx,cy,fillR,0,Math.PI*2);c.strokeStyle="rgba(159,247,255,.55)";c.lineWidth=.8;c.stroke()}}
